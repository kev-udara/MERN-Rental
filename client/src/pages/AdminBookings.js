...
import { getAllAdminBookings,updateBookingStatus } from '../redux/actions/bookingActions'
...
function AdminBookings() {
  ...
  const {bookings,carNames} = useSelector(state => state.bookingsReducer);
  const {loading} = useSelector((state)=> state.alertsReducer);
  const [filter, setFilter] = useState('');
  const [filterStatus, setFilterStatus] = useState('');
  const [sortOrder, setSortOrder] = useState('desc');
  const uniqueCarNames = [...new Set(bookings.map(({ car }) => car.name))];
  ...
  return (
    ...
    <br/> 
     ...
     <Row justify='center' gutter={16} style={{marginTop:'25px'}}>
      <Col lg={18} sm={24}>
...
<label className='text-center' style={{ color: 'rgb(254, 175, 56)',marginLeft:'12px' }}>Sort by:</label>
    ...
    {bookings
      .filter(({ car }) => (filter ? car.name === filter : true))
      ...
      .map((booking) => { 
              return <Row  gutter={16} className='adminbookingrow mt-3 p-2 text-left'  style={{backgroundColor:'rgb(254, 175, 56)',borderRadius:5}} key={booking._id}>
                <Col lg={6} sm={24} className='mt-2'>
                  <p style={{marginLeft:'8px'}}>User : <b>{booking.user.username}</b></p>
                  <p style={{marginLeft:'8px'}}>Car : <b>{booking.car.name}</b></p>
                ...
                <Col lg={4} sm={24} className='text-right'>
                  <img style={{borderRadius:15}} src={booking.car.image} alt='' height='150' className='p-2'/>
                </Col>
